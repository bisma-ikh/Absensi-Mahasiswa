            ...
            <div class="table-data">
				<div class="order">
					<div class="head">
                        ...
                        <a href="carijadwal.php" type="button" class="btn btn-outline-primary">Tambah Jadwal +</a>
					</div>
					<table>
						<thead>
						...
						<tbody>
							...
							$querymhs = "SELECT jadwal.*, 
												mata_kuliah.nama AS nama_mk, 
												mahasiswa.nama AS nama_mhs, 
												dosen.nama AS nama_dosen 
										 FROM jadwal
										 LEFT JOIN mata_kuliah ON jadwal.kode_mk = mata_kuliah.kode_mk
										 LEFT JOIN mahasiswa ON jadwal.nim = mahasiswa.nim
										 LEFT JOIN dosen ON jadwal.nip = dosen.nip
										 ORDER BY jadwal.created_at DESC";
							
							$resultmhs = mysqli_query($conn, $querymhs);
							while ($mhs = mysqli_fetch_assoc($resultmhs)) {
								echo "<tr>";
								echo "<td>" . mysqli_real_escape_string($conn, $mhs['kode_jadwal']) . "</td>";
								echo "<td>" . mysqli_real_escape_string($conn, $mhs['nama_mk']) . "</td>"; // Menampilkan nama mata kuliah
								echo "<td>" . mysqli_real_escape_string($conn, $mhs['nama_dosen']) . "</td>";
								echo "<td>" . mysqli_real_escape_string($conn, $mhs['nama_mhs']) . "</td>";
								echo "<td>" . mysqli_real_escape_string($conn, $mhs['ruang']) . "</td>";
								echo "<td>" . mysqli_real_escape_string($conn, $mhs['waktu']) . "</td>";
								echo "<td> <a style='color: var(--blue); font-size: 25px' href='editjadwal.php?id=" . mysqli_real_escape_string($conn, $mhs['kode_jadwal']) . "'>
												<i class='bx bx-edit'></i></a>
											</td>";
								echo "<td> <a style='color: var(--red); font-size: 25px; cursor: pointer;' onclick='confirmDelete(" . mysqli_real_escape_string($conn, $mhs['id']) . ")'>
    ...
    <script>
		...
		function confirmDelete(id) {
			...
			if (result) {
				...
				window.location.href = '../../../controller/app/admin/hapusjadwal.php?id=' + id;
			} else {
