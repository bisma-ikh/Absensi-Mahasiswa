            ...
            <div class="table-data">
				<div class="order">
					<div class="head">
                        ...
                        <a href="tambahdosen.php" type="button" class="btn btn-outline-primary">Tambah Dosen +</a>
					</div>
					<table>
						<thead>
						...
						<tbody>
							...
							$querymhs = "SELECT * FROM dosen ORDER BY created_at DESC";
							$resultmhs = mysqli_query($conn, $querymhs);
							while ($mhs = mysqli_fetch_assoc($resultmhs)) {
								echo "<tr>";
								echo "<td>" . mysqli_real_escape_string($conn, $mhs['nip']) . "</td>";
								...
								echo "<td> <a style='color: var(--blue); font-size: 25px' href='editdosen.php?id=" . mysqli_real_escape_string($conn, $mhs['id']) . "'>
							 				<i class='bx bx-edit'></i></a>
							 			</td>";
								echo "<td> <a style='color: var(--red); font-size: 25px; cursor: pointer;' onclick='confirmDelete(" . mysqli_real_escape_string($conn, $mhs['nip']) . ")'>
    ...
    <script>
		...
		function confirmDelete(id) {
			...
			if (result) {
				...
				window.location.href = '../../../controller/app/admin/hapusdosen.php?id=' + id;
			} else {
